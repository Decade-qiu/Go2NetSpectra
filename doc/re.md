...
**里程碑 3: API 服务与产品化 (Beta) - ⏳ 部分完成**

  * **目标**: 提供数据查询能力，并使系统易于部署。
  * **核心交付物**:
      * **`ns-api` 服务原型**: 已创建 `ns-api` 服务，可通过 HTTP POST 请求 `/api/v1/aggregate` 端点。
      * **双模式查询 API**: `ns-api` 服务现在提供两种查询能力：
          *   **聚合查询 (`/api/v1/aggregate`)**: 根据指定的过滤条件，返回每个任务的总字节数、总包数和独立流数量。
          *   **流追溯查询 (`/api/v1/flows/trace`)**: 根据指定的任务和流关键字，返回单个流的生命周期信息（首次/末次出现时间、总包数/字节数）。
      * **命令行查询工具**: 提供了一个位于 `scripts/query/` 的 Go 脚本，用于方便地通过 API 或直接连接数据库进行查询验证。

  * **下一步**:
      * 为 `ns-api` 增加日志中间件、CORS 支持和可配置的服务器地址。
