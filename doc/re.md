...
**里程碑 4: 分布式与高可用 (Release 1.0) - 🚧 进行中**

  * **目标**: 使系统具备生产环境的可靠性和扩展性。
  * **核心交付物**:
      * **高可用后端服务**: 通过 Kubernetes 的 `StatefulSet`，部署了多副本、高可用的 NATS 和 ClickHouse（使用内置 Keeper）集群。
      * **可水平扩展的应用服务**: `ns-api` 和 `ns-ai` 已配置为多副本部署，可通过 Kubernetes HPA (Horizontal Pod Autoscaler) 进一步实现自动扩缩容。
      * **Kubernetes 清单**: 在 `deployments/kubernetes/` 目录下提供了所有组件的 Kubernetes YAML 清单文件。
      * **一键部署脚本**: 提供了 `scripts/deploy-k8s.sh` 脚本，用于按正确顺序部署所有原始清单文件。
      * **Helm Chart**: 在 `deployments/helm/go2netspectra/` 目录下创建了完整的 Helm Chart，实现了生产级的、高度可配置的一键部署、升级和回滚能力。

  * **下一步**:
      * 完善 Helm Chart 的 Ingress 配置，以更优雅地暴露服务。
      * 编写完整的压力测试和稳定性测试方案。
