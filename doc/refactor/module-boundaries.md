...
| Transport contracts | `api/proto/v1`, `api/gen/v1` | Define protobuf transport schemas and generated client/server code | Runtime business logic or manual hotfixes in generated files |
...
| Query and API | `internal/api`, `internal/query`, `cmd/ns-api/v1`, `cmd/ns-api/v2` | Assemble HTTP/gRPC query servers and execute query workloads against persisted data | Probe capture logic or aggregator internals |
...
|---|---|
| `internal/api` | Shared server assembly for legacy HTTP, gRPC query, and Grafana-compatible HTTP routes |
...
3. `internal/probe` converts packets into protobuf transport messages and publishes to NATS.
4. `internal/engine/streamaggregator` consumes protobuf packets and forwards them to `internal/engine/manager`.
...
2. `pkg/pcap` reads offline traffic and emits protobuf transport packets.
...
3. `internal/ai` exposes the AI gRPC service used for enrichment.
